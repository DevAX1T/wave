...
async function userSearch(message, user) {
    // check if user is a number
    if (user instanceof User) {
        message.reply('discord '+user.tag)
    } else {
        let ruser = await noblox.getPlayerInfo(user).catch(e => {
            message.reply('Sorry! I could not find that user.');
        });
        message.reply(`${ruser.username} (${user.toString()})`);
...
module.exports = class LookupCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'p', // Lookup command - returns user accounts
            memberName: 'p',
            ...
            args: [
                {
                    key: 'user',
                    prompt: 'Who do you want to make smile??? :P',
    ...
    hasPermission(msg) {
        return PermissionManager.isModerator(msg.member) || PermissionManager.isModChannel(msg);
    }
